...
# Check if we have exactly 2 cameras after deduplication
if [ ${#final_cameras[@]} -ne 2 ]; then
    echo "⚠️  Expected 2 cameras after deduplication, found ${#final_cameras[@]}"
    echo "   This script is designed for exactly 2 cameras"
fi
...
if [ ${#serials[@]} -eq 2 ] && [ "${serials[0]}" != "${serials[1]}" ]; then
...
echo "2. Decide which camera should be 'front' and which should be 'side'"
